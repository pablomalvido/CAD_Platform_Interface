...
def get_path(p, end, length = 0, prev = 0):
    i=0
    straight = True
    while dist_2d(p[-1], end) > max(R_collision + step_increase, 0.025):
...
def plot_path(p, end, angle, offset):
    p_con = [cable_to_EE(p[0], angle, offset)]
    end_con = cable_to_EE(end, angle, offset)
    p_con, length, success = get_path(p_con, end_con)
    p_con.append(end_con)
    p_EE = []
    for p_con_i in p_con:
        p_EE.append(EE_to_cable(p_con_i, angle, offset))
...
def find_path_cb(req):
    ...
    for guide in guides:
        if guide.id == end_guide:
            ...
            plot_EP_x.append(guide.key_center_frame.position.x)
            plot_EP_y.append(guide.key_center_frame.position.y)
    ...
    wp = plot_path([starting_point], end_point, rotZ, length_cable[req.WH]) #Include orientation and Z
    if req.show:
        plt.show()
